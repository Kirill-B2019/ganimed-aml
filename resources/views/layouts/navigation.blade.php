...
<nav x-data="{ open: false }" class="sticky top-0 z-30 bg-white border-b border-slate-200">
    <div class="page">
        <div class="flex justify-between h-14">
            <div class="flex min-w-0">
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="flex items-center text-slate-900" aria-label="GANIMED AML">
                        <x-application-logo class="h-8 w-[12.25rem]" />
                    </a>
                </div>
                <div class="hidden sm:flex sm:items-stretch sm:ms-8 gap-5">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6 gap-3">
                <a href="{{ route('checks.create') }}" class="inline-flex items-center px-3 py-1.5 bg-indigo-600 text-sm font-medium text-white hover:bg-indigo-700 {{ request()->routeIs('checks.create') ? 'ring-2 ring-indigo-200' : '' }}">
                    {{ __('aml.new_check') }}
                </a>
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button type="button" class="inline-flex items-center px-2 py-1.5 text-sm text-slate-700 hover:text-slate-900">
            ...
            <div class="-me-2 flex items-center gap-2 sm:hidden">
                <a href="{{ route('checks.create') }}" class="inline-flex items-center px-2.5 py-1 bg-indigo-600 text-xs font-medium text-white">{{ __('aml.new_check') }}</a>
                <button type="button" @click="open = ! open" class="inline-flex items-center justify-center p-2 text-slate-500 hover:text-slate-800" :aria-expanded="open.toString()">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden border-t border-slate-200 bg-white">
        <div class="py-2">
            ...
            <div class="px-3 py-2 flex items-center justify-between">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="text-sm text-slate-600">{{ __('aml.log_out') }}</button>
                </form>
            </div>
        </div>
    </div>
